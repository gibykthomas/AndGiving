...
const columns: GridColDef[] = [
      ...
      return (
        <>
          ...
          <div
            className={
              params.row.id <= 3
                ? `${crown[params.row.id]}-img userListUser`
                : "userListUser"
            }
          >
            <div className="image-name">
              <img
                ...
                style={{
                  border: params.row.id <= 3 ? mediumBorder[params.row.id] : "",
                }}
...

  {
    ...
    flex: 3,
    renderCell: (params) => {
      return <>{parseInt(params.row.Total_Raised_to_date.replace(",", ""))}</>;
    },
  },
    ...
    renderCell: (params) => {
      return (
        <div className="amountRaised">
          ...
          <ProgressBar
            variant={"progress"}
            now={params.row.Match_requested / 6.5}
            label={`${Math.trunc(params.row.Match_requested / 6.5)}%`}
          />
        </div>
      );
    },
  },
  {
    ...
    renderCell: (params) => {
      return (
        <div className="amountRaised">
          <PopulateBadge
            val={params.row.Total_Raised_to_date.replace(",", "")}
          />
        </div>
      );
    },
...
export default function DataTable() {
  ...
  const sortData = data.sort(
    (a, b) =>
      b.Total_Raised_to_date.replace(",", "") -
      a.Total_Raised_to_date.replace(",", "")
  );
  ...
  React.useEffect(() => {
    if (data.length === 0) {
      axios
        .get(
          `https://sheet.best/api/sheets/87f43c47-3c43-4941-a9b2-f0569dafc098`
        )
        .then((res) => {
        ...
        .catch((error) => {
          ...
          this.setState({
            error: `There was an problem fetching the club data.`,
          });
        });
    }
  }, []);
